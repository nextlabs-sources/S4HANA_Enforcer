...
#define JUMP64_SIZE 28
#define TRAMPOLINE_SIZE (JUMP64_SIZE + JUMP64_SIZE + 16)
...
typedef struct funchook_entry {
    ...
    uint8_t old_code[200];
    uint8_t new_code[200]; //set this as max
#ifdef CPU_X86_64
    uint8_t transit[JUMP64_SIZE];
#endif
#ifdef CPU_PPC64
     uint8_t proxy_code[500]; //must be aligin with 4
    uint8_t _jump32ToNew;
    uint8_t _beginWithToc;
#endif
} funchook_entry_t;
...
int funchook_write_jump64(funchook_t *funchook, const uint8_t *src, const uint8_t *dst, uint8_t *out);
int funchook_within_32bit_relative(const uint8_t *src, const uint8_t *dst);
int funchook_jump32_avail(const uint8_t *src, const uint8_t *dst);
#endif
