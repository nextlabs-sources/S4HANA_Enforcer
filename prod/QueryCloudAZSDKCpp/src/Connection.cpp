...
CloudAZConnection::CloudAZConnection(const std::string& strBaseUrl, const std::string& strPort, const unsigned int msTimeout)
...
QueryStatus CloudAZConnection::CheckMultiResource(const IPolicyRequest** pcRequest, int nRequestCount, IPolicyResult** pcResult)
{
	...
	//std::string strLog = "CheckMultiResource request:" + strRequestBody;
	//QueryCloudAZMgr::Instance()->WriteLog(log_trace, strLog.c_str());

	http::request<http::string_body> req{ http::verb::post, pMgr->GetPDPPath().c_str(), 11, strRequestBody };
	...
	try
	{
        ...
        boost::shared_ptr<NXLHttpClientAsync> clt(new NXLHttpClientAsync(_server.c_str(),_port.c_str(),_ms_timeout));
        clt->SetBuffer(&req, &buffer, &res);
        QueryStatus status = clt->Run();
        if(status != QueryStatus::QS_S_OK){
            QueryCloudAZMgr::Instance()->WriteLog(log_warning, "NXLHttpClientAsync.run failed!");
            return  status;
		}
	}
	...
	//strLog = "CheckMultiResource response:" + ss.str();
	//QueryCloudAZMgr::Instance()->WriteLog(log_trace, strLog.c_str());
...
QueryStatus CloudAZConnection::CheckSingleResource(const IPolicyRequest* ceRequest, IPolicyResult** pcResult)
{
	...
	//std::string strLog = "CheckSingleResource request:" + strRequestBody;
	//QueryCloudAZMgr::Instance()->WriteLog(log_trace, strLog.c_str());
