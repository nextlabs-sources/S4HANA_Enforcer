...
//Init Policy sync
bool PolicyInit(const std::string &cchost, const std::string &ccport, const std::string &ccuser, const std::string &ccpwd, const std::string &model,
        unsigned int sync_interval_seconds, S4HException & /*exc*/, WriteLog_Fun log_fun ){
    if(log_fun){
        Log::m_logFun = log_fun;
    }
    return PolicyEngine::Ins()->Init(cchost, ccport, ccuser, ccpwd, model, sync_interval_seconds)==POLICY_ENGINE_SUCCESS;
}
