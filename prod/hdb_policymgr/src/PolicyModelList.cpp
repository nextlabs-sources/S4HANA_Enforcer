...
bool PolicyModelList::AddPmByID(uint64_t pmid, PolicyModel& out) {
     ...
     //when parse policy from file, talk=NULL
     if(!_talk){
        Log::WriteLog(log_warning, "PolicyModelList::AddPmByID, talk=NULL, pmid=%d", pmid);
        return false;
     }
...
bool PolicyModelList::AddPmByName(const std::string& name, PolicyModel& out) {
     ...
     //when parse policy from file, talk=NULL
     if(!_talk){
        Log::WriteLog(log_warning, "PolicyModelList::AddPmByName, talk=NULL, name=%s", name.c_str() );
        return false;
     }
...
bool PolicyModel::ParseFromJson(const char* json) {
    ...
    _type = get_pm_type(jsdata["type"].asString(),_name, _id);
