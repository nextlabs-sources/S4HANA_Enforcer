...
bool PolicyHelper::DownloadPolicys(const std::string& service, const std::string& port, const std::string& user, const std::string& pwd, const std::string& model_name,
                     std::vector<Json::Value>& rpolicys, PolicyModelList& rsymbols) {
    {
        ...
        std::string tg;
        bool r = talk->SearchPolicyIDsByTag(tg, pids);
        if (!r) {
            Log::WriteLog(log_info, "DownloadPolicys return false for SearchPolicyIDsByTag");
            delete (talk);
            return false;
        }
