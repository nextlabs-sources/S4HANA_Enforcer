...
TalkWithCC *TalkWithCC::MakeTalk(const std::string& service, const std::string& port, const std::string& user, const std::string& pwd) {
    ...
    if (!r) {
        Log::WriteLog(log_error, "TalkWithCC::MakeTalk GetCASLoginParameters failed");
        return nullptr;
    }
    assert(talk != nullptr);
    // step 2
    r = talk->LoginToCAS();
    if (!r) {
        Log::WriteLog(log_error, "TalkWithCC::MakeTalk LoginToCAS failed");
    ...
    if (!r) {
        Log::WriteLog(log_error, "TalkWithCC::MakeTalk CasSecurityCheck failed");
        delete (talk); talk = nullptr;
        return nullptr;
    }
    return talk;
...
bool TalkWithCC::GetCASLoginParameters(const std::string& service, const std::string& port,
    ...
    if (!r) {
       Log::WriteLog(log_fatal, "TalkWithCC::GetCASLoginParameters GetCASLoginUrl failed" );
      return false;
    }
    ...
    if (res.result() == http::status::ok) {
        ...
        //get CsrfToken, this introduce in CC9.1
        {
            ...
            if (nPosExecName!=std::string::npos) {
                ...
                Log::WriteLog(log_info, "TalkWithCC::GetCASLoginParameters create object TalkWithCC91" );
                return true;
            } else {
...
bool TalkWithCC91::CasSecurityCheck(http::response<http::string_body>& /*out*/) {
    ...
    if (!r){
         Log::WriteLog(log_fatal, "TalkWithCC91::CasSecurityCheck call parent failed.");
         return false;
    }
    if (res.result() == http::status::found) {
        auto& headers = res.base();
            ...
            if (!strXCSRFHdrName.empty()) {
                ...
                if (X_CSRF_TOKEN!=headers.end()) {
                    ...
                    return true;
                     ...
                     Log::WriteLog(log_fatal, "TalkWithCC91::CasSecurityCheck can't find header:%s", strXCSRFHdrName.c_str() );
                }


            }else{
                Log::WriteLog(log_fatal, "TalkWithCC91::CasSecurityCheck value of X-CSRF-HEADER is empty.");
            }
        }else{
            Log::WriteLog(log_fatal, "TalkWithCC91::CasSecurityCheck didn't find X-CSRF-HEADER header.");
        }

    }else{
         Log::WriteLog(log_fatal, "TalkWithCC91::CasSecurityCheck http status not:http::status::found.");
    }
    return false;
}
