...
//        AstExpr*sub_expr = parse_from_com_components(subject_components, ppmlst);
        ...
        AstExpr * pexp_sar = new AstBinaryOpExpr(AstExpr::AND, act_expr, res_expr );
...
bool Policy::GetPolicyBasicInfo(TablePolicyInfoMap & tbs_map, S4HException &/*exc*/){
    ...
    get_attribute_value_vec(tbs,  _expr, AstColumnRef::RES, "table" );
    ...
    get_attribute_value_vec(actions,  _expr, AstColumnRef::ACTION, "ACTION" );
    //bool values
    bool bfilter = false;
    bool bmask = false;
    bool bdup = false;
